...
$app->match('/admin/article/{id}/edit', function ($id, Request $request) use ($app) {
    ...
    if ($articleForm->isSubmitted() && $articleForm->isValid()) {
        $app['dao.article']->save($article);
        $app['session']->getFlashBag()->add('success', 'the article was succeffuly update');
    }
    return $app['twig']->render('article_form.html.twig', array(
        'title' => 'Edit article',
        'articleForm' => $articleForm->createView()));
...
$app->get('/api/articles', function() use ($app) {
    ...
    // create and return JSON reponse
...
$app->get('/api/article/{id}', function($id) use ($app) {
    $article = $app['dao.article']->find($id);
    // convert an objet ($article) into an associative array ($responseData)
    $responseData = array(
        'id' => $article->getId(),
        'title' => $article->getTitle(),
        'content' => $article->getContent()
...
$app->delete('/api/article/{id}', function($id, Request $reuqest) use ($app) {
    // delete all associated comments
    $app['dao.comment']->deleteAllByArticle($id);
    // delete the article
    $app['dao.article']->delete($id);
